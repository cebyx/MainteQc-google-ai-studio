...
export const ServiceHistoryView: React.FC = () => {
  ...
  const myTickets = tickets.filter(t => t.clientId === currentUser.id && (t.status === 'completed' || t.status === 'cancelled' || t.status === 'unable_to_complete'));
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">Service History</h1>
          <p className="text-gray-500 mt-1">View your past service records</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search history by title or address..."
            value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredTickets.map((ticket) => (
          <div 
            ...
            className="group rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-md hover:border-blue-200 cursor-pointer flex flex-col opacity-75 hover:opacity-100"
          >
            ...
            <div className="space-y-3 mb-6 flex-1">
              ...
              <div className="flex items-center gap-3 text-sm text-gray-600">
                ...
                <span>{ticket.scheduledDate ? formatDate(ticket.scheduledDate) : 'Unknown'}</span>
              </div>
            </div>
        ...
        {filteredTickets.length === 0 && (
          <div className="col-span-full py-12 text-center text-gray-500">
            No service history found matching your search.
          </div>
        )}
      </div>
